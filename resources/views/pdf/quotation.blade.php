...
<head>
    <meta charset="utf-8">
    <title>PROFORMA - {{ $quotation->number }}</title>
    <style>
        ...
        .doc-title h1 { color: #f97316; margin: 0; text-transform: uppercase; font-size: 24px; }
        ...
        .total-row.grand-total { border-top: 2px solid #f97316; font-weight: bold; font-size: 16px; color: #f97316; border-bottom: none; }
        .label { color: #6b7280; }
        .value { float: right; }
...
<body>
    <div class="header">
        ...
        <div class="client-info">
            <span class="label">DESTINATAIRE</span><br>
            <strong style="font-size: 15px;">{{ $quotation->prospect->name ?? $quotation->client->name ?? 'Client non spécifié' }}</strong><br>
            {{ $quotation->prospect->phone ?? $quotation->client->phone ?? '' }}<br>
            {{ $quotation->prospect->full_address ?? $quotation->client->address ?? '' }}
        </div>
        <div class="clear"></div>
    </div>
    ...
    <div class="doc-title">
        ...
        <p>Référence: <strong>{{ $quotation->number }}</strong> | Date: {{ $quotation->date->format('d/m/Y') }}</p>
    </div>
    ...
    <div class="totals">
        <div class="total-row">
            ...
            <span class="value">{{ number_format($quotation->subtotal, 0, ',', ' ') }} FCFA</span>
        </div>
        @if($quotation->discount_amount > 0)
        <div class="total-row">
            <span class="label">Remise globale</span>
            <span class="value">- {{ number_format($quotation->discount_amount, 0, ',', ' ') }} FCFA</span>
        </div>
        @endif
        @if($quotation->has_tax)
        <div class="total-row">
            <span class="label">TVA ({{ $quotation->tax_rate }}%)</span>
            <span class="value">{{ number_format($quotation->tax_amount, 0, ',', ' ') }} FCFA</span>
        </div>
        @endif
        <div class="total-row grand-total">
            <span>NET À PAYER</span>
            <span class="value">{{ number_format($quotation->total_amount, 0, ',', ' ') }} FCFA</span>
        </div>
        <p style="font-size: 11px; color:#6b7280; margin-top:10px;">
            Arrêté la présente facture proforma à la somme de : <br>
            <strong>{{ App\Services\SydoniaXmlService::numberToWords($quotation->total_amount) }} Francs CFA</strong>
        </p>
    </div>
